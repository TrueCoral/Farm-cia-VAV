
<?php require_once 'includes/Header.php'; ?>

<div class="content">
    <h2>Editar Produto</h2>

<?php
require(__DIR__ . '/config/conexao.php');

if (isset($_POST['buscar'])) {
    $id = $_POST['id'];

    $sql = "SELECT * FROM produtos WHERE id = :id";
    $stmt = $pdo->prepare($sql);
    $stmt->execute([':id' => $id]);
    $produto = $stmt->fetch(PDO::FETCH_ASSOC);

    if ($produto) {
?>

<form method="POST">
    <input type="hidden" name="id" value="<?= $produto['id'] ?>">

    Nome: <input type="text" name="nome" value="<?= $produto['nome'] ?>"><br><br>
    Preço: <input type="text" name="preco" value="<?= $produto['preco'] ?>"><br><br>
    Estoque: <input type="text" name="estoque" value="<?= $produto['estoque'] ?>"><br><br>
    Fabricante: <input type="text" name="fabricante" value="<?= $produto['fabricante'] ?>"><br><br>
    Dose: <input type="text" name="dose" value="<?= $produto['dose'] ?>"><br><br>

    <button type="submit" name="salvar">Salvar</button>
</form>

<?php
    } else {
        echo "<p>ID não encontrado</p>";
    }
}
?>

<?php
if (isset($_POST['salvar'])) {
    $sql = "UPDATE produtos SET 
        nome = :nome,
        preco = :preco,
        estoque = :estoque,
        fabricante = :fabricante,
        dose = :dose
        WHERE id = :id";

    $stmt = $pdo->prepare($sql);
    $stmt->execute([
        ':id' => $_POST['id'],
        ':nome' => $_POST['nome'],
        ':preco' => $_POST['preco'],
        ':estoque' => $_POST['estoque'],
        ':fabricante' => $_POST['fabricante'],
        ':dose' => $_POST['dose']
    ]);

    echo "<p>Produto atualizado!</p>";
}
?>

<form method="POST">
    <h3>Digite o ID para editar:</h3>
    <input type="number" name="id" required>
    <button type="submit" name="buscar">Buscar</button>
</form>

</div>

<?php require_once 'includes/Footer.php'; ?>

